...
import { TextInput, Select, FileInput, Label, Button } from "flowbite-react";
...
const CreateNoteForm: React.FC<CreateNoteFormProps> = ({ currentUser }) => {
  ...
  return (
    <div className="mb-4 rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-6 xl:p-8">
      ...
      <div
        className={`grid gap-4 ${currentUser ? "grid-cols-1" : "grid-cols-2"}`}>
        {!currentUser && (
          <div>
            <Label>User</Label>
            <div className="mt-1">
          ...
          <div className="mt-1">
            ...
            {file && (
              <div className="flex items-center justify-between">
                <span>{file.name}</span>
                <Button color="danger" onClick={() => setFile(null)}>
                  Remove
                </Button>
              </div>
            )}
        ...
        <Button
          ...
          disabled={loading}
          className={`w-full mt-4 ${currentUser ? "" : "col-span-2"}`}>
          Create Note
        </Button>
      </div>
    </div>
